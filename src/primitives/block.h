...
public:
    ...
    uint32_t nBits;
    uint32_t nNonce;
    uint256 hashStateRoot; // onix
    uint256 hashUTXORoot; // onix
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        READWRITE(nBits);
        READWRITE(nNonce);
        READWRITE(hashStateRoot); // onix
        READWRITE(hashUTXORoot); // onix
    ...
    void SetNull()
    {
        ...
        nBits = 0;
        nNonce = 0;
        hashStateRoot.SetNull(); // onix
        hashUTXORoot.SetNull(); // onix
    ...
    virtual bool IsProofOfStake() const //onix
    {
    ...
    CBlockHeader& operator=(const CBlockHeader& other) //onix
    {
        if (this != &other)
        {
...
public:
    ...
    std::pair<COutPoint, unsigned int> GetProofOfStake() const //onix
    {
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        block.nBits          = nBits;
        block.nNonce         = nNonce;
        block.hashStateRoot  = hashStateRoot; // onix
        block.hashUTXORoot   = hashUTXORoot; // onix
