...
void OnixDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool OnixDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule OnixDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t OnixDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t OnixDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t OnixDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t OnixDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool OnixDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void OnixDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void OnixDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void OnixDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void OnixDGP::createParamsInstance(){
...
dev::Address OnixDGP::getAddressForBlock(unsigned int blockHeight){
...
void OnixDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void OnixDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void OnixDGP::parseStorageOneUint64(uint64_t& value){
...
void OnixDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule OnixDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
