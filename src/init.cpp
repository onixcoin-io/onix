 ...
 */
static const char* BITCOIN_PID_FILENAME = "onixd.pid";
...
void Shutdown(NodeContext& node)
{
    ...
    util::ThreadRename("onix-shutoff");
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateOnix");
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path onixStateDir = GetDataDir() / "stateOnix";
                bool fStatus = fs::exists(onixStateDir);
                const std::string dirOnix(onixStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsOnixstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<OnixState>(new OnixState(dev::u256(0), OnixState::openDB(dirOnix, hashDB, dev::WithExisting::Trust), dirOnix, existsOnixstate));
                dev::eth::ChainParams cp(chainparams.EVMGenesisInfo());
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(onixStateDir.string()));
                if (fReset) {
            ...
            try {
                ...
                globalSealEngine->setOnixSchedule(onixDGP.getGasSchedule(::ChainActive().Height() + (::ChainActive().Height()+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
