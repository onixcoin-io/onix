...
#if defined(NDEBUG)
# error "Onix cannot be compiled without assertions."
#endif
...
std::unique_ptr<OnixState> globalState;
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        uint64_t minGasPrice = onixDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        uint64_t blockGasLimit = onixDGP.getBlockGasLimit(::ChainActive().Tip()->nHeight + 1);
        size_t count = 0;
        ...
        OnixTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractOnixTX resultConverter;
        if(!converter.extractionOnixTransactions(resultConverter)){
        ...
        std::vector<OnixTransaction> onixTransactions = resultConverter.first;
        std::vector<EthTransactionParams> onixETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(OnixTransaction onixTransaction : onixTransactions){
            sumGas += onixTransaction.gas() * onixTransaction.gasPrice();
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, onixTransaction.gasPrice());
            } else {
                txMinGasPrice = onixTransaction.gasPrice();
            }
            VersionVM v = onixTransaction.getVersion();
            if(v.format!=0)
            ...
            if(onixTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(onixTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(onixTransaction.gas() > UINT32_MAX)
            ...
            if(v.rootVM!=0 && (uint64_t)onixTransaction.gasPrice() < minGasPrice)
...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s ONIX additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // onix
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // onix
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount){
    ...
    OnixTransaction callTransaction;
    if(addrContract == dev::Address())
    {
        callTransaction = OnixTransaction(nAmount, 1, dev::u256(gasLimit), opcode, nonce);
    }
    else
    {
        callTransaction = OnixTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, nonce);
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<OnixTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path onixDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(onixDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(onixDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(OnixTransaction& tx : txs){
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{execRes, OnixTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries()), CTransaction()});
            continue;
        }
...
bool OnixTxConverter::extractionOnixTransactions(ExtractOnixTX& onixtx){
    ...
    std::vector<OnixTransaction> resultTX;
    ...
    onixtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool OnixTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool OnixTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
OnixTransaction OnixTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    OnixTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = OnixTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = OnixTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t OnixTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // onix
    OnixDGP onixDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setOnixSchedule(onixDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = onixDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = onixDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = onixDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // onix
...
///////////////////////////////////////////////////////////////////////////////////////// onix
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            OnixTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractOnixTX resultConvertOnixTX;
            if(!convert.extractionOnixTransactions(resultConvertOnixTX)){
            ...
            if(!CheckMinGasPrice(resultConvertOnixTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertOnixTX.first, blockGasLimit, pindex->pprev);
            ...
            for(OnixTransaction& qtx : resultConvertOnixTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertOnixTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertOnixTX.first[k].from(),
                        resultConvertOnixTX.first[k].to(),
                        ...
                        resultConvertOnixTX.first[k].getNVout(),
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 1000 && !Params().MineBlocksOnDemand()) {
        //sanity check in case an exploit happens that allows new coins to be minted
        if(pindex->nMoneySupply > (uint64_t)(108435435 + ((pindex->nHeight - 1000) * 4)) * COIN){
...
bool CChainState::ConnectTip(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // onix
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // onix
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // onix
            globalState->setRootUTXO(oldHashUTXORoot); // onix
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = ::ChainActive().Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // onix
        uint32_t sizeBlockDGP = onixDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Onix PoS we need to sync this after every block to ensure txdb is populated for
...
CAmount GetTxGasFee(const CMutableTransaction& _tx)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        OnixTxConverter convert(tx, NULL, NULL, contractflags);
