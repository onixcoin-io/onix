...
extern std::unique_ptr<OnixState> globalState;
...
inline void initState(){
    ...
    const std::string dirOnix = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<OnixState>(new OnixState(dev::u256(0), OnixState::openDB(dirOnix, hashDB, dev::WithExisting::Trust), dirOnix + "/onixDB", dev::eth::BaseState::Empty));
...
inline dev::Address createOnixAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline OnixTransaction createOnixTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    OnixTransaction txEth;
    if(recipient == dev::Address()){
        txEth = OnixTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = OnixTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<OnixTransaction> txs){
