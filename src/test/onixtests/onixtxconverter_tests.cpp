...
void checkResult(bool isCreation, std::vector<OnixTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    OnixTxConverter converter(transaction, NULL);
    ExtractOnixTX onixTx;
    BOOST_CHECK(converter.extractionOnixTransactions(onixTx));
    std::vector<OnixTransaction> result = onixTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(onixtxconverter_tests, TestingSetup)
