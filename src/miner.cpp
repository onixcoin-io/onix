...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setOnixSchedule(onixDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = onixDGP.getBlockSize(nHeight);
    minGasPrice = onixDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = onixDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    OnixTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractOnixTX resultConverter;
    if(!convert.extractionOnixTransactions(resultConverter)){
    ...
    for(OnixTransaction onixTransaction : onixTransactions){
        ...
        if(bceResult.usedGas + onixTransaction.gas() > softBlockGasLimit){
        ...
        if(onixTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, onixTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= checkpointSpan)
        {
            ...
            onixDelegations.FilterDelegationEvents(events, *this);
            delegations_staker = onixDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                onixDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                onixDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            onixDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            onixDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    OnixDelegation onixDelegations;
    int32_t cacheHeight;
    ...
    void Update(interfaces::Chain::Lock& locked_chain, int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= checkpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                onixDelegations.FilterDelegationEvents(events, *this);
                pwallet->m_my_delegations = onixDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    onixDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                    onixDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                onixDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(onixDelegations.GetDelegation(address, delegation) && OnixDelegation::VerifyDelegation(address, delegation))
                    {
...
public:
...

    {
        ...
        std::string threadName = "onixstake";
...
void StakeOnixs(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
