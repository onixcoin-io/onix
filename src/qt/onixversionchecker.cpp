...
#define paternVersion "onix-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

OnixVersionChecker::OnixVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

OnixVersionChecker::~OnixVersionChecker()
{

}

bool OnixVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> OnixVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(ONIX_RELEASES)));
    QEventLoop event;
...
Version OnixVersionChecker::getMaxReleaseVersion()
{
