...
Token::Token()
{
    ...
    lstMandatory.append(OnixToken::paramAddress());
    lstMandatory.append(OnixToken::paramDatahex());
    QStringList lstOptional;
    lstOptional.append(OnixToken::paramSender());
    ...
    lstMandatory.append(OnixToken::paramDatahex());
    lstOptional.clear();
    lstOptional.append(OnixToken::paramAmount());
    lstOptional.append(OnixToken::paramGasLimit());
    lstOptional.append(OnixToken::paramGasPrice());
    lstOptional.append(OnixToken::paramSender());
    lstOptional.append(OnixToken::paramBroadcast());
    lstOptional.append(OnixToken::paramChangeToSender());
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                Token::ToOnixAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
                Token::ToOnixAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
