...
#include <qt/onixversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    onixVersionChecker = new OnixVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and onix: URIs)"));
    ...
    QRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/qrctoken"), tr("&ORC Tokens"), this);
    QRCTokenAction->setStatusTip(tr("ORC Tokens (send, receive or add Tokens in list)"));
    ...
    openAction->setStatusTip(tr("Open a onix: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Onix command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && onixVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(ONIX_RELEASES, ONIX_RELEASES);
            QString message(tr("New version of Onix wallet is available on the Onix source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Onix network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
