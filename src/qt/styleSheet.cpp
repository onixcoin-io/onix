...
class OnixStyle : public QProxyStyle
{
public:
    OnixStyle()
    {
...
void StyleSheet::setStyleSheet(QApplication *app, const QString& style_name)
{
    ...
    OnixStyle* onixStyle = new OnixStyle;
    onixStyle->setBaseStyle(mainStyle);
    app->setStyle(onixStyle);
...
{
    return QStringList() << "theme1" << "theme2" << "theme3";
}

QStringList StyleSheet::getSupportedThemesNames()
{
    return QStringList() << "Dark theme (default)" << "Dark blue theme" << "Light blue theme";
}
