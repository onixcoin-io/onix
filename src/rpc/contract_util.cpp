...
UniValue transactionReceiptToJSON(const OnixTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
UniValue CallToContract(const UniValue& params)
{
    ...
    if(params.size() >= 3){
        CTxDestination onixSenderAddress = DecodeDestination(params[2].get_str());
        if (IsValidDestination(onixSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&onixSenderAddress);
    ...
    for(size_t i = 0; i < list.size(); i++)
    {
        ...
        for(size_t i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                ToOnixAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].get_str().substr(24);
                ToOnixAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
