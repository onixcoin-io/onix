Gitian building
================
*Setup instructions for a Gitian build of Onix Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Onix
...
- [Building Onix Core](#building-onix-core)
...
Clone the git repositories for onix and Gitian.
...
```bash
sudo cp /home/debian/onix/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the onix repository under 'Fetch and create inputs' to install sources which require
...
To build Onix Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the onix repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/onix/.git/
...
For example:
```bash
URL=https://github.com/laanwj/onix.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit onix=${COMMIT} --url onix=${URL} ../onix/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit onix=${COMMIT} --url onix=${URL} ../onix/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit onix=${COMMIT} --url onix=${URL} ../onix/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-onix:
./bin/gbuild --commit onix=${COMMIT},cpp-eth-onix=${ETHCOMMIT} --url onix=${URL},cpp-eth-onix=${ETHURL} ../onix/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the onix git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../onix/contrib/gitian-descriptors/*|sort|uniq )
...
./bin/gbuild --url onix=${BTCPATH},signature=${SIGPATH} ../onix/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/onix-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/onix-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/onix-osx-build.assert
```
